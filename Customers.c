...
CusNode* search(CusNode** Custree, int ID)
{
	if (!(*Custree))
	{
		return NULL;
	...
	if (ID < (*Custree)->CusID)
	{
		search(&((*Custree)->left), ID);
	}
	else if (ID > (*Custree)->CusID)
	{
		search(&((*Custree)->right), ID);
	}
	else if (ID == (*Custree)->CusID)
	{
		return *Custree;
